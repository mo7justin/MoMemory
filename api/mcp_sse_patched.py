...
class SseServerTransport:
    ...
    async def handle_post_message(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if not writer:
            # Auto-create session if it doesn't exist (for MCP SDK 1.17.4 SSE clients)
            logger.info(f"Session not found for ID: {session_id}, creating temporary session")
            read_stream_writer, read_stream = anyio.create_memory_object_stream(0)
            self._read_stream_writers[session_id] = read_stream_writer
            writer = read_stream_writer
            logger.debug(f"Created temporary session with ID: {session_id}")
